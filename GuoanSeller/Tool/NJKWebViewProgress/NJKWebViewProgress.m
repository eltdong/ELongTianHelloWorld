...
//NSString *LoginTimeOut = @"com=com_vpassport&method=login";
//NSString *HandleLoginOut = @"com=com_vpassport&method=logout";
NSString *completeRPCURLPath = @"";
NSString *LoginTimeOut = @"";
NSString *HandleLoginOut = @"";
...
- (void)setProgress:(float)progress
{
    ...
    if (progress > _progress || progress == 0 || progress == -1|| progress == -2) {
        _progress = progress;
...
#pragma mark -  
- (BOOL)custotmContainString1:(NSString *)str1 withStr2:(NSString *)str2{
...
{
    //用于判断主动点击退出按钮触发登出事件
//    [request.URL.absoluteString containsString:HandleLoginOut]
    NSLog(@"%@",request.URL.absoluteString);
    if([self custotmContainString1:request.URL.absoluteString withStr2:HandleLoginOut]){
        NSLog(@"-------------------");
        [self setProgress:-2];
        return NO;
    }
    ...
    if([self custotmContainString1:request.URL.absoluteString withStr2:LoginTimeOut]){
        [self setProgress:-1];
        return NO;
    }
