...
- (void)initValues
{
    ...
    //创建顶部可滑动的tab
    _topScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, SCREENWIDTH, kHeightOfTopScrollView)];
    ...
    UIView * lineView = [[UIView alloc]initWithFrame:CGRectMake(0, kHeightOfTopScrollView-1, SCREENWIDTH, 1)];
    lineView.backgroundColor = UIColorFromRGB(0xBCBCBC);
...
- (void)createNameButtons
{
    ...
    for (int i = 0; i < [_viewArray count]; i++) {
        ...
        CGSize textSize = [vc.title getcontentsizeWithfont:UIFont(15) constrainedtosize:CGSizeMake(150, 44) linemode:NSLineBreakByCharWrapping];
        if(textSize.width + 20 > topScrollViewContentWidth)
        topScrollViewContentWidth = (textSize.width + 20);
    }
    topScrollViewContentWidth = topScrollViewContentWidth * [_viewArray count];

    //每个tab偏移量
    CGFloat xOffset = widthofbutton;
    CGFloat width = topScrollViewContentWidth / [_viewArray count];
    //设置顶部滚动视图的内容总尺寸
    //    _topScrollView.contentSize = CGSizeMake(topScrollViewContentWidth, kHeightOfTopScrollView);
    _topScrollView.frame = CGRectMake(0, 0, topScrollViewContentWidth + 20, kHeightOfTopScrollView);
    _topScrollView.center = CGPointMake(self.bounds.size.width / 2, kHeightOfTopScrollView / 2);
    for (int i = 0; i < [_viewArray count]; i++) {
//        widthofbutton = width * i;
        UIViewController *vc = _viewArray[i];
        ...
        [button setTag:i+100];
        if (i == 0) {
            _shadowImageView.frame = CGRectMake(widthofbutton, 0, width, _shadowImage.size.height);
            button.selected = YES;
        }
...
//        [button setBackgroundImage:[UIImage imageNamed:@"tab_cr"] forState:UIControlStateSelected];
...
- (void)selectNameButton:(UIButton *)sender
{
    ...
    if (!sender.selected) {
        ...
        [UIView animateWithDuration:0.25 animations:^{
            ...
            [_shadowImageView setFrame:CGRectMake(sender.frame.origin.x, 10, sender.frame.size.width, _shadowImage.size.height)];
