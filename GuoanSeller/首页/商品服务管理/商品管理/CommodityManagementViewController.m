...
@interface CommodityManagementViewController ()<UITableViewDataSource,UITableViewDelegate,DropDownMenuViewDelegate,UITextFieldDelegate,UIAlertViewDelegate,AddCommodityViewControllerDelegate>
{
    ...
    NSString * taskDel;
    BOOL isView;
    AddCommodityViewController  * add;
    BOOL isModify;
    BOOL isSearch;
}
...
-(void)createUI{
    self.navbar.titleLabel.text = @"商品管理";
...
- (void)drop_on
{
    ...
    [self.tableView addLegendFooterWithRefreshingBlock:^{
        ...
        NSString * num = [NSString stringWithFormat:@"%ld",(long)_page];
...
//跳转到添加商品页
-(void)home:(id)sender{
    AddCommodityViewController *view = [[AddCommodityViewController alloc]init];
    view.isAddCommodity = YES;//添加
    view.shop_id = self.shop_id;
    view.member_id = self.manager_id;
    view.navibarTitle = @"添加商品";
    view.delegate = self;
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    CommodityManagement_sec_TableViewCell *cell2  = [tableView dequeueReusableCellWithIdentifier:cellID2];
    if (!cell2) {
        cell2 = [[[NSBundle mainBundle]loadNibNamed:@"CommodityManagement_sec_TableViewCell" owner:self options:nil]lastObject];
    }
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if ([self.type isEqualToString:@"20"]) {
        CommodityListModel * clModel = self.dataArr[indexPath.row];
        add = [[AddCommodityViewController alloc]init];
        add.clModel = clModel;
        ...
        CommodityListModel * clModel = self.dataArr[indexPath.row];
        AddServiceViewController *view = [[AddServiceViewController alloc]init];
...
- (IBAction)statusBtn:(id)sender {
    /**
     *  tag 11 12 13    1：上架，2:下架，3补货中 type
     */
...
//    [UIView animateWithDuration:0.5 animations:^{
//        [self.view layoutIfNeeded];//关闭动画 消除延迟
